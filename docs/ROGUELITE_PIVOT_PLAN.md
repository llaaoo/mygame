...
> 2026-07-15 更新：装备不再冻结。当前实现将装备改造为局内高影响构筑系统，保留九槽纸娃娃，但内容围绕词缀、套装阈值和技能标签联动设计；避免回到传统 RPG 的线性等级碾压。
...
- 不重写战斗系统。
- 不推翻 Runtime 架构。
- 不继续扩传统 RPG 长线任务和 NPC 日程；装备纸娃娃只服务局内构筑。
...
- 暂停扩展 RPG 任务和 NPC 日程；装备仅沿 roguelite 构筑方向扩展。
