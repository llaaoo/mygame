...
```
1. 新建 SkillData .tres  →  res://skills/data/xxx_data.tres
2. 新建场景 + 脚本       →  res://skills/scenes/xxx.tscn / xxx.gd
3. 注册到 SkillPool      →  在 _skill_pool.setup() 中 add_skill()
4. 配置到 Loadout        →  在 SkillLoadout .tres 中映射槽位
```

- PROJECTILE 类型：继承 `Projectile` 基类，覆写 `_apply_visual()`，设置 `projectile_scene` + `projectile_speed`
- BUFF 类型：新建 `Buff` .tres，设置 `buff_resource` + `buff_duration`
- AOE 类型：复用 `FlameStorm` / `IceExplosion` 模式（Area2D + 自毁），设置 `aoe_scene` + `aoe_radius`
- DASH 类型：参考 `ShadowStep`，设置 `dash_distance` + `dash_speed`
