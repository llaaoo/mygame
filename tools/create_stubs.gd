...
## 在旧路径创建桥接 .gd 文件（只放代码，不带 class_name，避免冲突）
## Godot 通过 UID 解析，.tscn 引用的是旧 UID，这里只保证文件存在
...
func _run() -> void:
	...
	for stub_path in stubs:
		var source_path: String = stubs[stub_path]
		if not FileAccess.file_exists(source_path):
			printerr("源文件不存在: ", source_path)
			continue
		DirAccess.make_dir_recursive_absolute(stub_path.get_base_dir())
		var src := FileAccess.open(source_path, FileAccess.READ)
		var content := src.get_as_text()
		src.close()

		# For .gd files: keep content but strip class_name to avoid conflicts
		# For .tres/.tscn: just copy
		if stub_path.ends_with(".gd"):
			# Remove class_name line, keep everything else
		...
		var out := FileAccess.open(stub_path, FileAccess.WRITE)
		out.store_string(content)
		out.close()
		print("✅ ", stub_path)

	print("\n=== 桥接文件创建完成 ===")
	print("现在所有旧路径都有文件，Godot 应该能正常加载")
	print("游戏可运行后，再逐步清理这些桥接文件")
